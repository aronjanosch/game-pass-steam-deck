...
def get_steam_users():
    ...
    for user_id in os.listdir(USERDATA_DIR):
        if user_id != "anonymous" and os.path.isdir(os.path.join(USERDATA_DIR, user_id)):
...
def modify_shortcuts_vdf(shortcuts_vdf, edge_app_id, new_name="Xbox Cloud Gaming (Beta)"):
    ...
    try:
        ...
        if matches:
            for match in matches:
                ...
                launch_options_pattern = re.compile(rb'\x01LaunchOptions\x00([^\x00]*)\x00')
                ...
                if launch_match:
                    ...
                    if b'--kiosk "https://www.xbox.com/play"' not in current_options:
                        new_options = current_options + b' ' + LAUNCH_OPTIONS.encode('utf-8')
                        content = content.replace(
...
def add_shortcut_to_steam(shortcuts_vdf, user_id):
    ...
    try:
        ...
        flatpak_info = run_command(["flatpak", "info", "com.microsoft.Edge"], check=False)
        ...
        new_entry = (
            b'\x00' + next_index.encode() + b'\x00' +
            b'\x02appid\x00' + app_id.to_bytes(4, byteorder='little') + b'\x00\x00\x00\x00' +
            b'\x01AppName\x00' + edge_name.encode() + b'\x00' +
            b'\x01Exe\x00"' + edge_exe.encode() + b'"\x00' +
            b'\x01StartDir\x00"' + os.path.dirname(edge_exe).encode() + b'"\x00' +
            b'\x01icon\x00\x00' +
            b'\x01ShortcutPath\x00\x00' +
            b'\x01LaunchOptions\x00' + edge_launch_args.encode() + b'\x00' +
...
def add_edge_to_steam():
    ...
    log("3. In the 'Add a Game' window, check Microsoft Edge and click 'Add Selected Programs'")
    ...
    log("Could not find Microsoft Edge in Steam after manual addition. Continuing anyway...")
    return None, None
...
def update_localconfig_vdf(user_id, edge_app_id, new_name="Xbox Cloud Gaming (Beta)"):
    ...
    try:
        localconfig_path = os.path.join(USERDATA_DIR, user_id, "config", "localconfig.vdf")
        ...
        app_section_pattern = re.compile(rf'"({edge_app_id}|Non-Steam-App_{edge_app_id})"\\s*{{')
...
def apply_artwork(edge_app_id, user_id):
    ...
    for art_type, url in ARTWORK.items():
        ...
        destination = os.path.join(ARTWORK_DIR, f"xbox_cloud_gaming_{art_type}{file_ext}")
...
╚═══════════════════════════════════════════════════╝
    ...
    steam_processes = run_command(["pgrep", "steam"], check=False)
    ...
    if edge_check and "com.microsoft.Edge" in edge_check.stdout:
        ...
        install_result = run_command(["flatpak", "install", "flathub", "com.microsoft.Edge", "-y"], check=False)
    ...
    udev_result = run_command(["flatpak", "--user", "override", "--filesystem=/run/udev:ro", "com.microsoft.Edge"], check=False)
